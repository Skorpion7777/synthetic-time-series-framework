...
from DataLoader import mitbih_train, anon_test
import cfg
...
class syn_mitbih(Dataset):
    def __init__(self, n_samples = 20000, seq_len = 187, reshape = False, model_path='./checkpoint'):
...
class mixed_anon(Dataset):
    def __init__(self, anon_samples = 200, syn_samples = 800):
        syn_ecg = syn_mitbih(n_samples=syn_samples, reshape=True)
        anon_ecg = anon_test(n_samples=anon_samples, oneD=True)
        
        if syn_ecg.data.shape[1] != anon_ecg.X_anonym.shape[1]:
            # Find the minimum number of columns
            min_cols = min(syn_ecg.data.shape[1], anon_ecg.X_anonym.shape[1])
            
            # Slice both arrays to have the same number of columns
            syn_ecg.data = syn_ecg.data[:, :min_cols]
            anon_ecg.X_anonym = anon_ecg.X_anonym[:, :min_cols]
        
        self.data = np.concatenate((syn_ecg.data, anon_ecg.X_anonym), axis = 0)
        self.labels = np.concatenate((syn_ecg.labels, anon_ecg.y_anonym), axis = 0)
